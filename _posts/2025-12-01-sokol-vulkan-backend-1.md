...
functions is very straightforward, e.g. the sokol-gfx API functions map
surprisingly well to Vulkan API functions, often the mapping is 1:1. This is
...
- memcpy the concrete descriptor blobs we stored upfront into the descriptor
  buffer to compose an adhoc descriptor set, using the offsets we also stored upfront
...
in the `sg_shader_desc` arg (which is usually code-generated by the sokol-shdc
shader compiler).

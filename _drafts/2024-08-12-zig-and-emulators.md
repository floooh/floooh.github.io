...
In the end I'm planning to develop the C and Zig emulators side by side for
...
- the same set of emulated systems should be supported
- all emulators should also run on the web via WASM with at least the same
  performance
- all emulators should feature an integrated debugger UI via Dear ImGui

Currently I'm still far away from that goal:

- only a handful emulated systems exist (KC85/2..4, Bomback, Pengo and Pacman)
- not yet running in browsers, although only because of minor features like
  loading game images into the emulator
- no debugging UI yet
...
On Mac, Windows or Linux, with a current stable or development Zig version installed:
...
...now build and run the currently implemented emulators:
...
...or run `zig build -l` to see a list of all run targets.

...more details are in the [README](https://github.com/floooh/chipz/blob/main/README.md)
...
- The entire emulation (including the CPU) is fully cycle-stepped. This
...
A typical chip emulator (including the CPU) basically only has one relevant
...
To implement a memory access, the 'system tick function' would check whether
the relevant bits are set for reading or writing a byte from or to memory:
...
function which takes a 'pins integer' as input and returns that same integer
with some pin bits modified. To know which chip currently 'owns the bus', a
process called 'address decoding' needs to happen which isn't much more than
checking for specific pin combinations being active and setting a unique 'chip
select' pin which signals to a specific chip emulator that it owns the shared
bus(ses) in the current clock cycle. This is (somewhat simplified) the magic
pixie which makes the emulators work.
...
(real hardware and FGPA-based emulators have the advantage that all of that
per-clock-cycle work can happens in parallel).
...
clock cycle, but actually less than half of that because we need wiggle room to
hit a consistent frame rate - also in debug mode - and we don't want to require
running the emulator on a high end CPU, so we're talking more like 100..200
host CPU clock cycles per emulated clock cycle.

Divide that further by the work that needs to be performed in
...
**TL;DR**: modern CPUs combined with modern compilers have a *really frigging
impressive* single-thread performance (even if it doesn't grow as much
...
- Getting rid of preprocessor defines for conditional compilation and 'bit twiddling'
  ...
  inline functions' to replace C helper macros (e.g. such functions are always inlined,
  even in debug code).
...
chips on a 64-bit unsigned integer.

For instance in the Z80 emulation, the 8-bit data bus might occupy bits 0..7,
the 16-bit address bus bits 7..23, and the remaining 'controls pins' like
`MREQ`, `IORQ`, `WR`, `RD`, `NMI` etc... would occupy bit positions
starting at 24.

When connecting other chip emulators to the Z80 CPU it makes sense to use
shared pin bit-positions to avoid having to shuffle bits around between calling
the chip emulator tick functions. For instance all chip emulators use the bit
positions 0..7 for the data bus, and also the same `IORQ` and `WR` positions
as the CPU while keeping clear of the other bit positions used by the Z80 emulations
to avoid pin collisions.

But this idea only goes so far for actual home computer systems. In a typical
home computer, there are not enough bits in a 64-bit integer to give each
chip its own pin range, even with some shared pin ranges. The other problem
is when a system has more than one chip of the same type (like the two VIAs
in a C64). With hardwired pin positions those two chip emulators would
collide.

This is why the original C emulators need a lot of code in the system tick
function to copy bits into temporary integers and move some of them into
the right position before calling the next chip emulator's tick function.
